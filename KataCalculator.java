...
public class KataCalculator {
    ...
    public static void main(String[] args) {
        ...
        if (isArab(num1) ^ isArab(num2)) {      //одно римское, одно арабское
            throw new IllegalStateException("Нельзя использовать римские и арабские числа одновременно");
        } else if (isArab(num1) & isArab(num2)) {     //оба арабских
            assert operation != null;
            int resArab = showResultArab(num1, num2, operation);
            System.out.println(resArab);
        } else if (!isArab(num1) & !isArab(num2)) {   //оба римских
            assert operation != null;
            String resRome = showResult(num1, num2, operation);
    ...
    public static String showResult(String num1, String num2, String operation) throws NumberFormatException {    //вычисление для римских
        ...
        switch (operation) {
            ...
            case "*" -> result = num1arab * num2arab;
            case "/" -> result = num1arab / num2arab;
            default -> throw new IllegalStateException("Неизвестный/Неправильный арефметический знак: " + operation);
